...
If the cameras are calibrated, we define the essential matrix:
\[
E = K^\top F K
\]
...
The physically valid solution is selected using the cheirality condition:
\begin{quote}
Triangulated points must lie in front of both cameras.
\end{quote}

Note: Only one candidate solution satisfies this constraint.

\section{Triangulation}

Given camera matrices \(P_1, P_2\) and corresponding image points \(x_1, x_2\), the 3D point \(X\) is recovered by solving
\[
x_i \times (P_i X) = 0, \quad i = 1,2
\]

Stacking the resulting equations yields the homogeneous linear system below, which is solved via SVD.
\[
A X = 0
\]
