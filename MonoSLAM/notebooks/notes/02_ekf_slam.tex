...
The Jacobian is a matrix of first-order partial derivatives that describes how a vector-valued function changes with respect to its input variables.
For a nonlinear function
\[
f: \mathbb{R}^n \to \mathbb{R}^m, \quad y = f(x)
\]
the Jacobian \(J_f(x)\) is defined as:
\[
J_f(x) =
\begin{bmatrix}
...
Each element \(\frac{\partial f_i}{\partial x_j}\) expresses how the \(i\)-th output changes with respect to the \(j\)-th input.
In the EKF, this matrix locally linearises nonlinear functions so that Gaussian uncertainty can be propagated through them.
...
\hat{z}_t = h(\hat{X}_t^-)
\]
where \(h\) is the measurement function which maps the state to sensor readings.

The innovation (residual) is:
\[
r_t = z_t - \hat{z}_t
\]
where \(z_t\) is the actual measurement received from the sensor.

The Kalman gain weights how much we trust the new measurement:
\[
K_t = P_t^- H_t^\top (H_t P_t^- H_t^\top + R_t)^{-1}
\]
where \(H_t\) is the Jacobian of the measurement function \(h\) evaluated at \(\hat{X}_t^-\), and \(R_t\) is the measurement noise covariance.
...
% ====
\section{3D Extension via Pinhole Projection Theorem}
% ====
